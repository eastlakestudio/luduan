...
public struct UserProgressModel: Codable, Equatable {
    ...
    /// 多源数据安全融合：只增不减，确保版本升级和多端同步时数据不丢失
    public mutating func merge(with other: UserProgressModel) {
        self.completedLevelIds.formUnion(other.completedLevelIds)
        self.learnedPhrases.formUnion(other.learnedPhrases)
        self.unlockedBadgeIds.formUnion(other.unlockedBadgeIds)
        self.freshReplayThemeIds.formUnion(other.freshReplayThemeIds)
        self.totalScore = max(self.totalScore, other.totalScore, self.learnedPhrases.count * 10)
    }
}
